...
export async function generateTrainingPlan(
  ...
  if (!apiKey) {
    throw new Error("OPEN ROUTER KEY is not set in environment variables");
  }
  ...
  try {
    ...
    if (!content) {
      ...
      throw new Error("No content in respose");
    }
    ...
    console.error("(AI) Error generating Training Plans", error);
    throw error;
  }
}
...
function formatPlanResponse(aiResponse: any, profile: UserProfile):Omit<TrainingPlan, "id" | "userId" | "version" | "createdAt"> {
    const plan: Omit<TrainingPlan, "id" | "userId" | "version" | "createdAt">  = {
        overview: {
            goal: aiResponse.overview?.goal || `Customized ${profile.goal} plan`,
            frequency:
